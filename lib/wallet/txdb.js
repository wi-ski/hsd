...
class TXDB {
  ...
  putBid(b, nameHash, outpoint, options) {
    ...
    assert(typeof acct === undefined || typeof acct === number)
    ...
    bb.own = options.own;
    b.put(layout.i.encode(nameHash, hash, index, acct), bb.encode());
  }
  ...
  async getBids(nameHash) {
    // Thinking new account val in layout has this is borked now.
    const iter = this.bucket.iterator({
